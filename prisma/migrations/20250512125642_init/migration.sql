/*
  Warnings:

  - The primary key for the `detail` table will be changed. If it partially fails, the table could be left without primary key constraint.
  - You are about to drop the column `id` on the `detail` table. All the data in the column will be lost.
  - The required column `id_detail` was added to the `detail` table with a prisma-level default value. This is not possible if the table is not empty. Please add this column as optional, then populate it before making it required.

*/
-- AlterTable
ALTER TABLE "detail" DROP CONSTRAINT "detail_pkey",
DROP COLUMN "id",
ADD COLUMN     "id_detail" TEXT NOT NULL,
ADD CONSTRAINT "detail_pkey" PRIMARY KEY ("id_detail");
